                                        ...
                                        Http::ResponseWriter* response )
{
    ...
    const string venue_name = field_map.at ( "venue" );
    const string image_csv_names = field_map.at ( "frame" );
    const string camera_model_name = field_map.at ( "camera_model" );
    const string camera_params_csv = field_map.at ( "camera_params" );
    ...
    if ( localizers_.find ( venue_name ) == localizers_.end() ) {
        localizers_[venue_name] = make_shared<Localizer> ( venue_name, azure_blob_loader_ );
    }

    // Localize images.
    std::shared_ptr<Localizer> localizer = localizers_.at ( venue_name );
    std::future<std::pair<int, std::string>> future = std::async ( std::launch::async, &Localizer::Localize, localizer, camera_model_name, camera_params_csv, image_names );
