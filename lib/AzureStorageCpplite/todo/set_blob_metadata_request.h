...
#include "AzureStorageCpplite/storage_request.h"
#include "AzureStorageCpplite/process_storage_request.h"
#include "AzureStorageCpplite/sign_storage_request.h"
#include "AzureStorageCpplite/storage_url.h"
#include "AzureStorageCpplite/utility.h"
#include "AzureStorageCpplite/constants.h"
#include "AzureStorageCpplite/common.h"

namespace azure {
    namespace storage_lite {
        namespace experimental {

            struct set_blob_metadata_request : blob_request {
                ...
                std::map<std::string, std::string> meta() { return std::map<std::string, std::string>(); }
            };
            ...
            struct storage_request_processor<Request, Http, std::enable_if_t<std::is_base_of<set_blob_metadata_request, std::remove_reference_t<Request>>::value>> {
                static inline void process(Request &&r, Http &&h) {
                    ...
                    std::string method = constant::http_put;
                    h.set_method(http_method::put);
                    ...
                    for (auto it = m.begin(); it != m.end(); it++) {
                        details::add_ms_header(h, ms_headers, constant::header_ms_meta_prefix + it->first, it->second);
                    }
